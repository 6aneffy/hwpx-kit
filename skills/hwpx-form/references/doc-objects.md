...
- **쪽번호는 한글 COM으로 실측** (Windows + 한글이면 `--pages auto` 기본값이
  알아서) — 이 단계에서 **한글이 백그라운드로 잠깐 뜨고 한컴 보안 경고
  팝업이 날 수 있다**(우리 도구가 한글을 자동 조종하는 정상 신호, 파일 이상
  아님)고 사용자에게 미리 알리고 시작할 것. 한글 없는 환경/쪽번호 불필요면
  `--pages none`으로 제목만 (팝업 없음)
...
(진단용 `open-check`는 한글을 자동 구동해 보안 경고 팝업을 유발하니 평소엔
쓰지 말 것 — 개발·회귀 전용.)
