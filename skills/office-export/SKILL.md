...
전제: `officecli` CLI (선택 의존성). 없으면 공식 설치 안내 —
Windows PowerShell `irm https://d.officecli.ai/install.ps1 | iex` (인터넷 필요).
officecli가 없어도 DOCX는 `hwpx-kit export`만으로 가능(후처리 생략), PPTX/XLSX는 불가.
