...
👤 "이 이미지 표처럼 그려줘"              → 병합·색·열폭·정렬 갖춘 표 원샷 생성
```
...
hwpx-kit table-build 문서.hwpx --spec 표스펙.json --at-text "앵커" --out 결과.hwpx  # 표 원샷 생성 (병합·색·열폭·정렬·내용)
hwpx-kit cell-merge 문서.hwpx --table 3 --range "0,0:0,2" --out 결과.hwpx     # 셀 병합 (해제는 cell-split)
hwpx-kit cell-color 문서.hwpx --table 3 --range "0,0:0,5" --color "#D9E5FF" --out 결과.hwpx  # 셀 배경색
...
- 실사용 시나리오 테스트 다수 — 백지 생성 완주, 장·표 동적 추가, 보도자료 변환,
  외부 사용자 설치·작성 검증 (발견된 문제는 즉시 수정 반영)
